...
class Actor(nn.Module):
    def __init__ (self, state_dim, action_dim, max_action):
        ...
        self.cnn_base = nn.Sequential(  # input shape (1, 42, 42)
                nn.Conv2d(1, 8, kernel_size=(3,3), padding = 0),
                nn.BatchNorm2d(8),
                #nn.Dropout(p=0.2),
                #nn.LeakyReLU(0.1),
                nn.ReLU(),  # activation
                ...
                nn.Conv2d(32, 8, kernel_size=(3,3), padding = 0),
                nn.BatchNorm2d(8),  
                #nn.Dropout(p=0.2),
                #nn.LeakyReLU(0.1),
                nn.ReLU(),  # activation
                ...
                nn.Conv2d(16, 8, kernel_size=(3,3), padding = 0),
                nn.BatchNorm2d(8), 
                #nn.Dropout(p=0.2),
                nn.ReLU(),
                #nn.LeakyReLU(0.1),
                ...
                nn.Conv2d(in_channels=16, out_channels=8, kernel_size=(1, 1), padding=0),
                nn.AvgPool2d(3)                 
        )

        
        # Defining the first Critic neural network
        self.layer_1 = nn.Sequential(nn.Linear(8+2, 300))
        self.layer_2 = nn.Sequential(nn.Linear(300, 100))
...
class Critic(nn.Module):
    ...
    # Defining the first Critic neural network
        ...
        self.cnn_base2 = nn.Sequential(  # input shape (4, 96, 96)
              nn.Conv2d(1, 8, kernel_size=(3,3), padding = 0),
                nn.BatchNorm2d(8),
                #nn.Dropout(p=0.2),
                #nn.LeakyReLU(0.1),
        ...
        self.action_dim = action_dim
        # Defining the first Critic neural network
        self.layer_1 = nn.Linear(8 + self.action_dim +2, 300)
        self.layer_2 = nn.Linear(300, 100)
        self.layer_3 = nn.Linear(100, 1)

        # Defining the Second Critic neural network
        self.layer_4 = nn.Linear(8 + self.action_dim+2, 300)
        self.layer_5 = nn.Linear(300, 100)
...
class TD3(object):
  ...
  def __init__(self, state_dim, action_dim, max_action):
    ...
    self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=0.0002,weight_decay = 0.001)
    self.critic = Critic(state_dim, action_dim)
    self.critic_target = Critic(state_dim, action_dim)
    self.critic_target.load_state_dict(self.critic.state_dict())
    self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=0.0002, weight_decay = 0.001)
    self.max_action = max_action
